    ...
    const Fr_t& alpha, const Fr_t& beta, const Fr_t& C, const vector<Fr_t>& u)
{
    ...
    if (phase1_err != cudaSuccess) {
        throw std::runtime_error(std::string("CUDA error after tLookup_phase1_poly_kernel: ") + cudaGetErrorString(phase1_err));
    }
    ...
    const vector<Fr_t>& u, const vector<Fr_t>& v1, const vector<Fr_t>& v2, vector<Polynomial>& proof)
{
    if (!v1.size())
    {
        ...
        Fr_t expected = p({0, 0, 0, 0, 0, 0, 0, 0}) + p({1, 0, 0, 0, 0, 0, 0, 0});
        if (!fr_approx_equal(claim, expected)) throw std::runtime_error("tLookup_phase1: claim != p(0) + p(1)");
        ...
        if (reduce_err != cudaSuccess) {
            throw std::runtime_error(std::string("CUDA error after tLookup_phase1_reduce_kernel: ") + cudaGetErrorString(reduce_err));
        }
