...
zkSoftmax_v2::zkSoftmax_v2(const vector<uint>& bs, uint L, uint M, unsigned long scaling_factor_in, const vector<double>& thetas, uint m, uint n, uint d, uint E):
...
KERNEL void zksoftmax_decompose_kernel(const Fr_t* X, const uint* bs, Fr_t* X_decomposed, uint N, uint K)
{
    ...
    // Convert to signed long (handles both positive and negative field elements correctly)
    long x_val = scalar_to_long(X[gid]);
    
    // We want to decompose -x (negative values become positive for decomposition)
    unsigned long tmp = static_cast<unsigned long>(-x_val);
    
    // DEBUG: Print first element
    if (gid == 0) {
        printf("KERNEL DEBUG: X[0] field = %u %u, x_val = %ld, tmp = %lu\\n", 
               X[0].val[1], X[0].val[0], x_val, tmp);
    }
...
// input should be m * n
FrTensor zkSoftmax_v2::compute(const FrTensor& X, FrTensor& shift, FrTensor& X_shifted, vector<FrTensor>& X_segments, vector<FrTensor>& Y_segments, vector<FrTensor>& m_segments)
{
    ...
    cudaError_t decompose_err = cudaGetLastError();
    if (decompose_err != cudaSuccess) {
        throw std::runtime_error (std::string("CUDA error after zksoftmax_decompose_kernel:") + cudaGetErrorString(decompose_err));
    }
    ...
    // DEBUG: Check out after initialization
    Fr_t* out_cpu_init = new Fr_t[min(5u, out.size)];
    cudaMemcpy(out_cpu_init, out.gpu_data, min(5u, out.size) * sizeof(Fr_t), cudaMemcpyDeviceToHost);
    std::cerr << "DEBUG zkSoftmax: out after init, first 5: ";
    for (uint i = 0; i < min(5u, out.size); i++) {
        std::cerr << out_cpu_init[i] << " ";
    }
    std::cerr << std::endl;
    delete[] out_cpu_init;
    ...
    if (init_err != cudaSuccess) {
        throw std::runtime_error (std::string("CUDA error after FrTensor out init:") + cudaGetErrorString(init_err));
    }
    ...
    {   
        ...
        if (memcpy_err != cudaSuccess) {
            throw std::runtime_error (std::string("CUDA error after cudaMemcpy for segment ") + std::to_string(i) + ": " + cudaGetErrorString(memcpy_err));
        }
        
        auto counts = least_significant_segments[i].prep(X_segments[i]);
        m_segments.push_back(counts);
        
        cudaError_t prep_err = cudaGetLastError();
        if (prep_err != cudaSuccess) {
            throw std::runtime_error (std::string("CUDA error after prep for segment ") + std::to_string(i) + 
                ": [code " + std::to_string(prep_err) + "] " + cudaGetErrorString(prep_err));
        }
    }
    ...
    for (uint i = L; i < K; ++ i)
    {
        ...
        cudaError_t memcpy_err2 = cudaGetLastError();
        if (memcpy_err2 != cudaSuccess) {
            throw std::runtime_error (std::string("CUDA error after cudaMemcpy for other segment ") + std::to_string(i) + ": " + cudaGetErrorString(memcpy_err2));
        }
        
        auto p = other_segments[i - L](X_segments[i]);
        
        cudaError_t other_seg_err = cudaGetLastError();
        if (other_seg_err != cudaSuccess) {
            throw std::runtime_error (std::string("CUDA error after other_segments call for segment ") + std::to_string(i) + ": " + cudaGetErrorString(other_seg_err));
        }
        ...
        // DEBUG: Check out after multiplying segment i
        Fr_t* out_cpu_seg = new Fr_t[min(5u, out.size)];
        cudaMemcpy(out_cpu_seg, out.gpu_data, min(5u, out.size) * sizeof(Fr_t), cudaMemcpyDeviceToHost);
        std::cerr << "DEBUG zkSoftmax: out after segment " << i << ", first 5: ";
        for (uint j = 0; j < min(5u, out.size); j++) {
            std::cerr << out_cpu_seg[j] << " ";
        }
        std::cerr << std::endl;
        delete[] out_cpu_seg;
        
        cudaError_t mult_err = cudaGetLastError();
        if (mult_err != cudaSuccess) {
            throw std::runtime_error (std::string("CUDA error after multiply for segment ") + std::to_string(i) + ": " + cudaGetErrorString(mult_err));
        }
    }
...
Fr_t zkSoftmax_v2::prove(const FrTensor& Y, const FrTensor& X, const FrTensor& shift, const FrTensor& X_shifted,
    ...
    vector<Polynomial>& proof)
{
    ...
    // DEBUG
    cout << "DEBUG: X_seg_claims[0] = " << X_seg_claims[0] << endl;
    cout << "DEBUG: bs.size = " << bs.size() << ", K = " << K << endl;
...
zkAttn_v2::zkAttn_v2(unsigned long sf_Q, unsigned long sf_K, const vector<uint>& bs, uint L, uint M, const vector<double>& thetas, uint m, uint n, uint d, uint E):
zkSoftmax_v2(bs, L, M, sf_Q * sf_K, thetas, m, n, d, E) {}

FrTensor zkAttn_v2::compute(const FrTensor& Q, const FrTensor& K, const FrTensor& V, FrTensor& sm_in, FrTensor& sm_out,
    FrTensor& sm_shift, FrTensor& sm_in_shifted, vector<FrTensor>& sm_in_segments, vector<FrTensor>& sm_out_segments, vector<FrTensor>& sm_m_segments)
{
    ...
    sm_out = zkSoftmax_v2::compute(sm_in, sm_shift, sm_in_shifted, sm_in_segments, sm_out_segments, sm_m_segments);
    return FrTensor::matmul(sm_out, V, m, n, d);
}

vector<Claim> zkAttn_v2::prove(const FrTensor& Q, const FrTensor& K, const FrTensor& V, const FrTensor& out,
        ...
        const vector<FrTensor>& sm_in_segments, const vector<FrTensor>& sm_out_segments, const vector<FrTensor>& sm_m_segments)
{
    ...
    cout << "zkAttn_v2 proof complete." << endl;
    return {};
}

Fr_t zkAttn_v2::prove(const FrTensor& Q, const FrTensor& K, const FrTensor& V, const FrTensor& out,
        const FrTensor& sm_out, const FrTensor& sm_in, const FrTensor& sm_shift, const FrTensor& sm_in_shifted,
        const vector<FrTensor>& sm_in_segments, const vector<FrTensor>& sm_out_segments, const vector<FrTensor>& sm_m_segments,
        ...
        vector<Polynomial>& proof)
{
    ...
    auto sm_in_claim = zkSoftmax_v2::prove(sm_out, sm_in, sm_shift, sm_in_shifted, sm_in_segments, sm_out_segments, sm_m_segments,
...
zkAttnStacked_v2::zkAttnStacked_v2(uint num, unsigned long sf_Q, unsigned long sf_K, const vector<uint>& bs, uint L, uint M, const vector<double>& thetas, uint m, uint n, uint d, uint E): 
num(num), zkAttn_v2(sf_Q, sf_K, bs, L, M, thetas, m, n, d, E) {}

Fr_t zkAttnStacked_v2::prove(const FrTensor& Q, const FrTensor& K, const FrTensor& V, const FrTensor& out,
        const FrTensor& sm_out, const FrTensor& sm_in, const FrTensor& sm_shift, const FrTensor& sm_in_shifted,
        const vector<FrTensor>& sm_in_segments, const vector<FrTensor>& sm_out_segments, const vector<FrTensor>& sm_m_segments,
